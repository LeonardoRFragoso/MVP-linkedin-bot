...
from backend.bot.modules.helpers import make_directories
from backend.bot.config.settings import run_in_background, stealth_mode, disable_extensions, safe_mode, file_name, failed_file_name, logs_folder_path, generated_resume_path
from backend.bot.config.questions import default_resume_path
if stealth_mode:
...
from backend.bot.modules.helpers import find_default_profile_directory, critical_error_log, print_lg
...
try:
    ...
    # Add arguments to avoid conflicts with other Chrome instances
    options.add_argument("--remote-debugging-port=9223")  # Use different port
    options.add_argument("--no-sandbox")
    options.add_argument("--disable-dev-shm-usage")

    print_lg("IF YOU HAVE MORE THAN 10 TABS OPENED, PLEASE CLOSE OR BOOKMARK THEM! Or it's highly likely that application will just open browser and not do anything!")
    
    # Create dedicated profile directory for LinkedIn bot to avoid conflicts
    import os
    linkedin_bot_profile = os.path.join(os.getcwd(), "../../data/chrome_profile")
    if not os.path.exists(linkedin_bot_profile):
        os.makedirs(linkedin_bot_profile)
    options.add_argument(f"--user-data-dir={linkedin_bot_profile}")
    print_lg(f"Using dedicated Chrome profile: {linkedin_bot_profile}")
    if stealth_mode:
        # try: 
